...
public class ToeicSlidingUpPanelLayout extends ViewGroup {
    
    private static final String TAG = ToeicSlidingUpPanelLayout.class.getSimpleName();
    ...
    public ToeicSlidingUpPanelLayout(Context context) {
        this(context, null);
    }

    public ToeicSlidingUpPanelLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ToeicSlidingUpPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ToeicSlidingUpPanelLayout);
 
            if (ta != null) {
                mPanelHeight = ta.getDimensionPixelSize(R.styleable.ToeicSlidingUpPanelLayout_showedSize, -1);
                mShadowHeight = ta.getDimensionPixelSize(R.styleable.ToeicSlidingUpPanelLayout_shadowSize, -1);
 
                mMinFlingVelocity = ta.getInt(R.styleable.ToeicSlidingUpPanelLayout_flingVelocity, DEFAULT_MIN_FLING_VELOCITY);
                mCoveredFadeColor = ta.getColor(R.styleable.ToeicSlidingUpPanelLayout_fadeColor, DEFAULT_FADE_COLOR);
 
                mDragViewResId = ta.getResourceId(R.styleable.ToeicSlidingUpPanelLayout_dragView, -1);
            }
...
//            Log.e(TAG, "onMeasure: More than two child views are not supported.");
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean finalResult = interceptForDrag || interceptTap;
        if (!finalResult){
//        	 mDragHelper.cancel();
